    ...
    usage(FILE * out)
{
	...
	/*
kilobytes和kibibytes是两种不同的单位，用于表示数字信息的大小。它们的区别如下：

kilobyte（简写为kB）是基于十进制的单位，它的前缀kilo表示1000（103），因此，一个kilobyte等于1000个字节（byte）。
这个单位通常用于表示网络传输速率、存储介质容量等方面的数据量。

kibibyte（简写为KiB）是基于二进制的单位，它的前缀kibi表示1024（210），因此，一个kibibyte等于1024个字节（byte）。
这个单位通常用于表示内存容量、CPU缓存大小等方面的数据量。

使用kilobyte和kibibyte的原因是，由于计算机使用二进制来处理和存储数据，因此，使用2的幂次方来表示数据量比较方便。
但是，由于1024和1000相差不大，所以在早期的计算机科学中，人们习惯用kilobyte来表示1024个字节，而不是1000个字节。
这样就造成了混淆和误解。

为了解决这个问题，国际电工委员会（IEC）在1998年制定了一套新的单位前缀，例如kibi、mebi、gibi等，来明确地表示2的幂次方。

总之，kilobyte和kibibyte的主要区别是：
kilobyte是基于十进制的单位，一个kilobyte等于1000个字节。
kibibyte是基于二进制的单位，一个kibibyte等于1024个字节。
kilobyte和kibibyte之间有7.37%的差异。
	*/
...
int main(int argc, char **argv)
{
	...
	do {
        ...
        // 是否需要显示total信息，此total合并mem和swap
		if (flags & FREE_TOTAL) {
			printf("%-7s", _("Total:"));
